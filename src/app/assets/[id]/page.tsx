...
import Asset from "@/app/components/assets/asset";
...
export default function AssetPage({}: Props) {
  ...
  useEffect(() => {
    ...
    if (id !== "new") {
      (async () => {
        setLoading(true);
        const path = user.roles.includes("buyer") ? "buyers" : "sellers";
        const asset = await fetch(`/api/${path}/${user.id}/assets/${id}`).then(
          (res) => res.json()
        );
  ...
  return (
    <>
      {loading ? (
        ...
        <Asset asset={asset} bidAction={bidAction} />
      )}
    </>
  );
}
